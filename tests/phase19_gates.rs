...
use zeroclaw::security::policy::{CommandRiskLevel, SecurityPolicy};
...
fn gate2_unauthorized_user_cannot_approve() {
    ...
    let result = registry.resolve("11111111", "eve_the_attacker", true);
...
fn gate3_wrong_role_cannot_approve_high_risk() {
    ...
    let result = registry.resolve("22222222", "regular_user", true);
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("not authorized"));
}
...
fn gate4_timeout_invalidates_request() {
    ...
    let result = registry.resolve("33333333", "user_1", true);
    assert!(result.is_err());
...
fn gate5_concurrent_approvals_independent() {
    ...
    // Approve request A
    let result_a = registry.resolve("aaaa0001", "user_a", true);
    assert!(result_a.is_ok());
    assert!(result_a.unwrap().contains("Approved"));

    // Deny request B
    let result_b = registry.resolve("bbbb0002", "user_b", false);
...
fn gate6_callback_replay_rejected() {
    ...
    let first = registry.resolve("44444444", "user_1", true);
    assert!(first.is_ok());

    // Second attempt fails (entry removed after first resolve)
    let second = registry.resolve("44444444", "user_1", false);
...
fn gate7_cross_request_isolation() {
    ...
    let result = registry.resolve("bbbb2222", "user_a", true);
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("not authorized"));

    // Request A is still pending and resolvable by user_a
    let result = registry.resolve("aaaa1111", "user_a", true);
    assert!(result.is_ok());
}
...
fn gate20_deny_returns_denied_message() {
    ...
    let result = registry.resolve("55555555", "user_1", false);
    assert!(result.is_ok());
